...
const AUDIT_AGE_MS = 24 * 60 * 60 * 1000;
...
const TOOLS = [
  ...
  { name: "audit_question_for_correction", title: "修正前監査", description: "指定した大問・フィールドを監査し、現在の原文・ハッシュ・検出事項を固定した24時間有効の一回限り監査IDを発行します。問題データ自体は変更しません。修正にはこの監査を先に実行する必要があります。", inputSchema: { type: "object", properties: { exam_id: { type: "integer", minimum: 1 }, question_number: { type: "integer", minimum: 1 }, target_field: { type: "string", enum: ["problem_text", "answer_text", "translation_text", "commentary_text"] }, reason: { type: "string", minLength: 5, maxLength: 500, description: "監査・修正が必要な理由" } }, required: ["exam_id", "question_number", "target_field", "reason"], additionalProperties: false } },
