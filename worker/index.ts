    ...
    messages: [
      {
        ...
        content: [
          ...
          { type: "text", text: `添付の入試問題PDFを解析し、大問ごと・セクションごとに構造化してください。${hintText}\n\n以下のJSON形式のみで出力してください。前後に説明文やコードブロックを含めないでください。\n{"universityName":"...","year":2024,"schedule":"...","questions":[{"questionNumber":1,"category":"...","sections":[{"type":"問題","text":"..."},{"type":"解答","text":"..."},{"type":"解説","text":"..."},{"type":"全訳","text":"..."}]}]}` },
        ],
      },
    ],
  };
        ...
        if (ingestCfg.replacements.length && parsed && Array.isArray(parsed.questions)) {
          for (const q of parsed.questions) {
            if (Array.isArray(q.sections)) {
              for (const s of q.sections) s.text = applyReplacements(s.text || "", ingestCfg.replacements);
            }
            // 旧フィールド（後方互換）
            if (typeof q.problemText === "string") q.problemText = applyReplacements(q.problemText, ingestCfg.replacements);
            if (typeof q.answerText === "string") q.answerText = applyReplacements(q.answerText, ingestCfg.replacements);
            if (typeof q.commentaryText === "string") q.commentaryText = applyReplacements(q.commentaryText, ingestCfg.replacements);
          }
        }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (path === "/api/config" && request.method === "GET") {
        ...
        const defaultSectionTypes = ["問題", "解答", "解説", "全訳"];
        return json({
