...
- 語注（脚注・注記・「注)」の列挙）は **##語::訳##** タグを、注釈対象の語の直後に埋め込む形で本文中に書く（末尾に訳一覧が自動生成される）。例えば原文が `The immune system※ plays a key role※2 in...（※免疫システム ※2役割）` なら → `The ##immune system::免疫システム## plays a key ##role::役割## in...` のように、対象語を ##語::訳## で直接囲む。注記を「注)〜」などの行として別途出力しない。
